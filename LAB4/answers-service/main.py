...
logger = logging.getLogger("answers-service")
...
def answer_question(
    ...
    student_url = f"http://students-service:8004/students/{student_id}"
    student_response = requests.get(student_url)
    if student_response.status_code != 200:
        raise HTTPException(status_code=400, detail="Студента з таким ID не існує")

    # Формуємо URL для отримання даних запитання
    question_url = f"http://questions-service:8003/questions/{question_id}"
    logger.info(f"Requesting question data from: {question_url}")
